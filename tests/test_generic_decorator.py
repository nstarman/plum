...
def test_decorator_with_parentheses() -> None:
    @generic()
    class Parens(Generic[T]):
        def __init__(self, value: Any) -> None:
            self.value = value
    ...
    assert Parens(1).__orig_class__ == Parens[int]
...
def test_decorated_dispatch_is_served_by_tier_one() -> None:
    """Bare construction routes through the `GENERIC` aspect, and caches."""
    dispatch = Dispatcher()
