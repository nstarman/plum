...
def test_unhashable_target_still_raises():
    """An unhashable `type_to` raises a `TypeError`.

    The memo lookup hashes `type_to`, as the method lookup behind it already does,
    so it must raise rather than swallow the failure.
    """
