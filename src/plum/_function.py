...
class _ModuleDescriptor(str):
    ...
    Instance access reports whatever the wrapped function has, `None` included.
    ...
    def __get__(self, instance: Any, owner: type) -> str | None:
        # `None` is not a fallback but the real answer: a function's `__module__` is
        # `globals().get("__name__")`, so it is `None` for one defined by `exec` in a
        # namespace without `__name__` (a doctest under Sybil, say), and CPython reports
        # exactly that. Mirroring it keeps `_split_parts` (which already skips a `None`
        # module) and `functools.wraps` — used by `classmethod` — working as they do on
        # the wrapped function. Annotating this `str` made a `mypyc`-compiled build
        # raise `TypeError` where the pure build returned `None`.
        module: str | None = getattr(instance._f, "__module__", None)
        return module
...
class _BoundFunction:
    ...
    def __init__(self, f: "Function", instance: object) -> None:
        ...
        # come from the descriptors attached below.
        _wraps(self, f._f)
